...
public class NeedToAdmInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		boolean isCastingDirector = (boolean) request.getAttribute("isCastingDirector");
		...
		if (isAdmin == false && isCastingDirector == false ) {
			if (isAjax == false) {
				response.setContentType("text/html; charset=UTF-8");
				response.getWriter().append("<script>");
				response.getWriter().append("alert('캐스팅 디렉터로 로그인 후 다시 이용해주세요.');");
				...
				response.getWriter().append("{\"resultCode\":\"F-B\",\"msg\":\"캐스팅 디렉터로 로그인 후 다시 이용해주세요.\"}");
			}
			return false;
		}
