...
public class ISNIRecord {
	...
	@XmlElement(name = "creationClass")
	public String creationClass;
	
	@XmlElement(name = "titleOfWork")
	public List<TitleOfWork> titleOfWorks;
	...
	public List<Source> getSources() {
		if (source == null) {
			...
			if (sources1 != null)
				source.addAll(sources1);
			if (sources2 != null)
				source.addAll(sources2);
		}
		return source;
	}
	...
	public List<TitleOfWork> getTitleOfWorks() {
		if (titleOfWorks == null)
			titleOfWorks = new ArrayList<>();
		return titleOfWorks;
	}
	...
	public static ISNIRecord fromFile(Path file) throws IOException, JAXBException {
		...
		if (!fileContent.startsWith("<ISNIAssigned>"))
			fileContent = IsniSearchService.splitBody(fileContent).get(0);
	...
	public static ISNIRecord fromString(String record) throws JAXBException {
		...
		record = record.replaceAll("</?creativeActivity>", "");
	...
	public boolean hasName(String forename, String surname, boolean _default) {
		...
		for (PersonalName pn : personalNames) {
			...
			if (surname.equalsIgnoreCase(pn.surname) && forename.equalsIgnoreCase(pn.forename))
				return true;
		}
		return false;
	}
